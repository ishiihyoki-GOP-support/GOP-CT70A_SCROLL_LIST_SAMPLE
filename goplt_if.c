...
/*
	...
	戻り値	sum値
*/
static uint8_t _calc_sum(uint8_t *cmd)
{
	uint8_t sum=0;
...
/*
	...
	備考	戻り値は次に本関数を呼び出すまで有効
			...
			とする
*/
static uint8_t *byte_tohexstr(uint8_t b)
{
...
/*
	...
	戻り値	変換された値
*/
static uint8_t hexstr_tobyte(uint8_t *bstr)
{
	uint8_t ret=0;
	int i;
	for(i=0;i<2;i++,bstr++)
...
static uint8_t puts_sum(uint8_t *cmd){
	uint8_t sum;
	sum=_calc_sum(cmd);
	ltputs(cmd);
	return sum;
...
//標準関数非依存対策
static int _strlen(uint8_t *buf) {
	int ret = 0;
	while (buf[ret] != 0)ret++;
	return ret;
}
...
#else
				vals[i] = hexstr_tobyte((uint8_t *)s);
				s += 2;
...
static ltMem_Callback* mem_callback = NULL;
...
char* goi_ini_data;
char* meminfo_txt_data;
char* pagedata_dat_data;
char* GB0000_gb_data;
char* GB0001_gb_data;
char* SI0001_lsd_data;
char* SI0002_lsd_data;
...
static int FileUpload(char *src,char *dest,int size)
{
	FILE *rf;
	struct _stat sbuf;
	int rlen;
	...
	if(rf){
		...
		while(1){
			...
			if(rs!=0){
				int i;
				buf[0]=0x02;
				buf[1]=(addr>>0 )&0x000000ff;
				buf[2]=(addr>>8 )&0x000000ff;
				buf[3]=(addr>>16)&0x000000ff;
				buf[4]=(addr>>24)&0x000000ff;
			...
			now=get_syscount();
			while (!(cmd = gets_lt(cmd_buf, sizeof(cmd_buf), TIMEOUT)));
			if (cmd[0] != 0x06||get_syscount()>now+FASTUPLOAD_TIMEOUT) {
				goto err;
			}
